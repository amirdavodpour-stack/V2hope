...
// Fixed-format dummy hash used to equalize the expensive password verification path for unknown accounts.
// It is never associated with a real user and is intentionally not a usable credential.
export const DUMMY_PASSWORD_HASH = 'pbkdf2$600000$o9CABYT9tLYo9xCe0dtWDw$UG20uWD8MNhIYimnvwcIZoGDHYvBWLF5zazdqESoAVw';
...
export function passwordNeedsRehash(encoded) {
  try {
    const [scheme, iterations, salt, expected] = String(encoded).split('$');
    return scheme !== 'pbkdf2' || Number(iterations) !== PBKDF2_ITERATIONS || !salt || !expected;
  } catch {
    return true;
  }
}
